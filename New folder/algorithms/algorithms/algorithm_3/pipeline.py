...
This implements global histogram equalization combined with CLAHE (Contrast
Limited Adaptive Histogram Equalization) for fingerprint contrast improvement.
...
PIPELINE_BUILD = "histogram-equalization-v1"


def _histogram_enhance(gray: np.ndarray, foreground: np.ndarray) -> np.ndarray:
    """Apply global histogram equalization followed by aggressive CLAHE."""
    fg = foreground.astype(bool)

    # Stage 1: Global histogram equalization
    equalized = cv2.equalizeHist(gray)

    # Stage 2: CLAHE with aggressive settings for ridge emphasis
    clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
    clahe_result = clahe.apply(equalized)

    # Stage 3: Unsharp mask for edge sharpening
    blurred = cv2.GaussianBlur(clahe_result.astype(np.float32), (0, 0), 1.5)
    sharpened = cv2.addWeighted(clahe_result.astype(np.float32), 1.5, blurred, -0.5, 0)
    output = np.clip(sharpened, 0, 255).astype(np.uint8)

    # Only apply enhancement to foreground
    result = gray.copy()
    result[fg] = output[fg]
    return result
...
def run_algorithm(
    ...
    *,
    ...
    """
    Enhance a fingerprint using Histogram Equalization + CLAHE.
    ...
    enhanced = _histogram_enhance(preprocessed, foreground_mask)
